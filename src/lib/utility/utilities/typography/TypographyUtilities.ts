...
export const TextDecorationThicknessTransformer = (thickness: string) =>
    `${thickness}px`;
...
export const TextDecorationTransformer = (suffix: string) => {
    ...
    return `${line} ${ColorTransformer(color)} ${style} ${thickness}px`;
}
...
const TypographyUtilities: Utilities = {
    ...
    ...OneActionUtility(GhostPrefix.TEXT_SHADOW, PropertyCSS.TEXT_SHADOW),
};
